// pages/Dashboard.jsx
// FIX: Student dashboard stat cards were broken. The original code did:
//   Object.entries(STATUS_LABELS).filter(([key]) => key === 'published').map(() =>
//     Object.entries(data.typeCounts).map(...)
//   )
// This is a nested .map() inside a .filter().map() returning arrays of arrays,
// not a flat list of JSX. React renders nothing for arrays-of-arrays in this
// context. Replaced with a direct Object.entries(data.typeCounts).map().
// FIX: Removed archive status from content overview display
...
const STATUS_COLORS = {
  ...
  // archived:         'from-purple-400 to-indigo-500', // REMOVED
};
...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      {user.role !== 'student' && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            {/* Content status breakdown - ARCHIVED REMOVED */}
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
              ...
              <div className="space-y-3">
                {/* FIX: Filter out 'archived' status */}
                {Object.entries(data.contentCounts)
                  .filter(([status]) => status !== 'archived') // ← REMOVED ARCHIVE
                  .map(([status, count]) => {
                    const total = Object.values(data.contentCounts)
                      .filter((_, idx) => Object.keys(data.contentCounts)[idx] !== 'archived')
                      .reduce((a, b) => a + b, 0);
              ...
              {user.role === 'hod' && data.contentCounts.pending_approval > 0 && (
                ...
                >
                  Review {data.contentCounts.pending_approval} Pending Item
                  {data.contentCounts.pending_approval !== 1 ? 's' : ''}
      ...
      {user.role === 'student' && (
        <>
          {/* FIX: direct .map() over typeCounts — no nested filter/map */}
